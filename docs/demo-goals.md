...
- 不需要原生手機 App。
- 不需要完整商業化 UI。
- 不需要 3,000 人壓測在每台電腦上都穩定跑滿；初期以 500 到 1,000 人現場展示，再用 3,000 人設定與截圖補充。
...
- 系統的核心壓力來源是高頻位置更新。
- 不需要真的找 500 到 1,000 位同學測試，進階壓測也可把參數調到 3,000 人。
- 壓測腳本可重現高併發情境。
...
- `--users 300` 已預演成功。
- `--users 500` 與 `--users 1000` 已預演成功。
- `--users 3000` 作為進階挑戰參數，不作為初期必要成功條件。
...
- 是否能展示 Pod 被刪除後自動恢復。
- 是否能說明 500-1,000 虛擬使用者如何產生高併發流量，以及如何把參數提高到 3,000。
- 是否能把日常校園情境和後端架構連在一起。
...
如果 1,000 或 3,000 人壓測使電腦太卡：

- 改用 500 人現場展示。
- 說明初期目標是 500-1,000 人，3,000 人是進階挑戰或截圖補充。
